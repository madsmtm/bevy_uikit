...
for arch in $ARCHS; do
  ...
  LIPO_ARGS+="$CARGO_TARGET_DIR/$triple/$CARGO_XCODE_BUILD_PROFILE/examples/$EXAMPLE_NAME
"
done
...
{ cargo build --features="${CARGO_XCODE_FEATURES:-}" $CARGO_XCODE_TARGET_FLAGS $OTHER_INPUT_FILE_FLAGS --example $EXAMPLE_NAME --verbose --message-format=short 2>&1 | sed -E 's/^([^ :]+:[0-9]+:[0-9]+: error)/\1: /' >&2; } || { echo >&2 "error: cargo-xcode project build failed; $CARGO_XCODE_TARGET_TRIPLES"; exit 1; }
...
DEP_FILE_DST="$DERIVED_FILE_DIR/$ARCHS-$EXAMPLE_NAME.d"
echo "" > "$DEP_FILE_DST"
for triple in $CARGO_XCODE_TARGET_TRIPLES; do
  BUILT_SRC="$CARGO_TARGET_DIR/$triple/$CARGO_XCODE_BUILD_PROFILE/examples/$EXAMPLE_NAME"

  # cargo generates a dep file, but for its own path, so append our rename to it
  DEP_FILE_SRC="$CARGO_TARGET_DIR/$triple/$CARGO_XCODE_BUILD_PROFILE/examples/$EXAMPLE_NAME.d"
